...
const TeacherManagement = () => {
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      let res;
      if (editingTeacher) {
        res = await api.put(`/api/teachers/${editingTeacher._id}`, formData);
      } else {
        res = await api.post('/api/teachers', formData);
      }
  ...
  const handleDeleteTeacher = async (id) => {
    if (window.confirm('Are you sure you want to delete this teacher? This will clear the teacher assignment in the class.')) {
      try {
  ...
  const fields = [
    ...
    { name: 'phone', label: 'Phone Number', type: 'text', required: true, placeholder: 'e.g. +1 555 123 4567' },
    { name: 'salary', label: 'Monthly Salary (₹)', type: 'number', required: true, placeholder: 'e.g. 5000', min: 0 },
    { 
      name: 'assignedClass', 
    ...
    { name: 'address', label: 'Residential Address', type: 'textarea', required: true, placeholder: 'Enter home address details...', fullWidth: true }
  ];
  ...
  const columns = [
    ...
    { 
      ...
      key: 'assignedClass', 
      sortable: false,
      render: (row) => row.assignedClass?.name || <span className="text-slate-400">None</span>
    },
    {
      header: 'Actions',
      key: 'actions',
    ...
    <div className="min-h-screen pl-64 pt-20">
      ...
      <main className="p-8 flex flex-col gap-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200 font-display">Faculty Members</h3>
            <p className="text-xs text-slate-400 dark:text-slate-500">Manage school educators, salaries, and contacts</p>
          </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-lg shadow-2xl border border-slate-100 dark:border-slate-800 flex flex-col gap-5">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold font-display text-slate-800 dark:text-slate-200">
                  {editingTeacher ? 'Edit Teacher Details' : 'Add Teacher Record'}
                </h3>
                <button 
